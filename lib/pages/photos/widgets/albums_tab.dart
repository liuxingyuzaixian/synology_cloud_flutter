...
class AlbumsTab extends StatefulWidget {
  const AlbumsTab({super.key, this.isSharedSpace = false});

  final bool isSharedSpace;

  @override
  State<AlbumsTab> createState() => AlbumsTabState();
}

class AlbumsTabState extends State<AlbumsTab> {
  ...
  Future<void> _loadAlbums() async {
    ...
    final cached = AppPreferences.getString('cache_photos_albums');
    ...
    try {
      final res = await (widget.isSharedSpace ? DsmApi().fotoTeamAlbums(offset: 0, limit: 100) : DsmApi().fotoAlbums(offset: 0, limit: 100));
      if (res['success'] == true) {
        final list = (res['data']?['list'] as List?) ?? [];
        _albums.clear();
        _albums.addAll(list.map((e) => FotoAlbum.fromJson(e)));
        // Save to cache
        AppPreferences.putString('cache_photos_albums', jsonEncode(list));
      } else {
        _error = '加载失败';
      }
    } catch (e) {
  ...
  Widget build(BuildContext context) {
    ...
    if (_albums.isEmpty) {
      return Center(
        child: Text('暂无相册', style: TextStyle(fontSize: 14.asp, color: Colors.grey)),
      );
    }
  ...
  void _openAlbum(FotoAlbum album) {
    Navigator.of(context).push(
      MaterialPageRoute(
        builder: (_) => _AlbumDetailPage(album: album, isSharedSpace: widget.isSharedSpace),
      ),
    );
  }
}
...
class _AlbumCard extends StatelessWidget {
  const _AlbumCard({required this.album, required this.onTap, required this.isSharedSpace});
                ...
                child: thumbUrl.isNotEmpty
                    ? CachedNetworkImage(
                        ...
                        errorWidget: (context, _, __) => Container(
                          ...
                          child: Center(
                            child: Icon(Icons.photo_library_outlined, color: Colors.grey[400], size: 40.r),
                          ),
                        ),
                      )
                    : Container(
...
class _AlbumDetailPage extends StatefulWidget {
  const _AlbumDetailPage({required this.album, required this.isSharedSpace});
  final FotoAlbum album;
  final bool isSharedSpace;
...
class _AlbumDetailPageState extends State<_AlbumDetailPage> {
  ...
  Future<void> _loadPhotos() async {
    setState(() => _loading = true);
    try {
      final res = await (widget.isSharedSpace ? DsmApi().fotoTeamBrowseItem(offset: 0, limit: _limit, albumId: widget.album.id) : DsmApi().fotoBrowseItem(offset: 0, limit: _limit, albumId: widget.album.id));
      if (res['success'] == true) {
        final list = (res['data']?['list'] as List?) ?? [];
        _photos.clear();
  ...
  Future<void> _loadMore() async {
    ...
    try {
      final res = await (widget.isSharedSpace ? DsmApi().fotoTeamBrowseItem(offset: _offset, limit: _limit, albumId: widget.album.id) : DsmApi().fotoBrowseItem(offset: _offset, limit: _limit, albumId: widget.album.id));
      if (res['success'] == true) {
        final list = (res['data']?['list'] as List?) ?? [];
